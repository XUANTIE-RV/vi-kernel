...
#include "ov2778_reg_cfg.h"

#define SENSOR_CLK 51000000

static struct vvcam_mode_info pov2778_mode_info[] = {
	{
		.index     = 0,
		.width     = 1920,
		.height    = 1080,
...
static int32_t sensor_query(void *ctx, struct vvcam_mode_info_array *pmode_info_arry)
{
	...
	pmode_info_arry->count = ARRAY_SIZE(pov2778_mode_info);

	memcpy(pmode_info_arry->modes,pov2778_mode_info,sizeof(pov2778_mode_info));
...
static int32_t sensor_init(void *ctx, struct vvcam_mode_info *pmode)
{
	...
	for (i=0; i < sizeof(pov2778_mode_info)/ sizeof(struct vvcam_mode_info); i++)
	{
		if (pov2778_mode_info[i].index == pmode->index)
        {
            psensor_mode = &(pov2778_mode_info[i]);
            break;
        }
	}
	...
	{
		case 0:
			ret = sensor_write_reg_arry(ctx,&ov2778_mipi4lane_1080p_30fps_linear_arry);
...
struct vvcam_sensor_function_s ov2778_function =
{
	.sensor_name = "ov2778",
	.reserve_id  = 0x2770,

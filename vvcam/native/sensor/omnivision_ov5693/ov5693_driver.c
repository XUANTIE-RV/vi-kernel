...
#include "ov5693_reg_cfg.h"

#define SENSOR_CLK 51000000

static struct vvcam_mode_info pov5693_mode_info[] = {
	{
		.index     = 0,
		.width     = 1920,
		.height    = 1080,
...
static int32_t sensor_query(void *ctx, struct vvcam_mode_info_array *pmode_info_arry)
{
	...
	pmode_info_arry->count = ARRAY_SIZE(pov5693_mode_info);

	memcpy(pmode_info_arry->modes,pov5693_mode_info,sizeof(pov5693_mode_info));
...
static int32_t sensor_init(void *ctx, struct vvcam_mode_info *pmode)
{
	...
	for (i=0; i < sizeof(pov5693_mode_info)/ sizeof(struct vvcam_mode_info); i++)
	{
		if (pov5693_mode_info[i].index == pmode->index)
        {
            psensor_mode = &(pov5693_mode_info[i]);
            break;
        }
	}
	...
	{
		case 0:
			ret = sensor_write_reg_arry(ctx,&ov5693_mipi4lane_1080p_30fps_linear_arry);
...
static int32_t sensor_set_stream(void *ctx, uint32_t status)
{
	...
	if (status)
	{
        ret = sensor_write_reg(ctx, 0x100, 0x01);
	}else
	{
        ret = sensor_write_reg(ctx, 0x100, 0x0);
	}
...
struct vvcam_sensor_function_s ov5693_function =
{
	.sensor_name = "ov5693",
	.reserve_id  = 0x5600,
